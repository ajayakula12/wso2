...
public class RetryFlowMediator extends AbstractMediator  {
	...
	public boolean mediate(MessageContext messageContext) {
		...
		try
		{
			List<ApiLogHandlerDto> requestLogApiInfo=new RetryApiConfigDetails().getApiRequestLogDetails(sourceRequestID);
			...
			List<ApiResponseDTO> apiMasterList=new GetApiConfigDetails().getMasterApiConfigDetails(requestApiInfo.getApi_id(), "queryType", "tmfParams");
             if(apiMasterList!=null)
			{
				for (int i = 0; i < apiMasterList.size(); i++) {
					...
					if(apiResponseObj==null )						
					{
						...
						clientResponse = makePostRequest(sourceRequestID,DBinfoConstants.UOL_DYNAMIC_SEQ_API, apiInfoObj,requestApiInfo.getRequest(),aggregateResponse.toString());
